...
stepRange=13
indexName="NDX"
# NDX is not actively trade

# Inputs and Parameters
filename = "/Users/yiuminglai/GitProjects/hkstockoption/streamlit/test/" + 'ndx_quotedata.csv'
...
fromStrike = spotPrice - step * stepRange
toStrike = spotPrice + step * stepRange
...
chartTitle = "Total Gamma: $" + str("{:.2f}".format(df['TotalGamma'].sum())) + " Bn per 1% " + indexName + " Move"
...
plt.axvline(x=spotPrice, color='r', lw=1, label=indexName + " Spot: " + str("{:,.0f}".format(spotPrice)))
plt.legend()
plt.show()
...
df.to_csv("/Users/yiuminglai/GitProjects/hkstockoption/streamlit/test/data/" + "gamma.csv")
...
chartTitle = "Gamma Exposure Profile, "+indexName+", " + todayDate.strftime('%d %b %Y')
plt.title(chartTitle, fontweight="bold", fontsize=20)
...
chartTitle = "Gamma Profile, "+indexName+", " + todayDate.strftime('%d %b %Y')
plt.title(chartTitle, fontweight="bold", fontsize=20)
plt.xlabel('Index Price', fontweight="bold")
plt.ylabel('Gamma Exposure ($ billions/1% move)', fontweight="bold")
plt.axvline(x=spotPrice, color='r', lw=1, label=indexName+" Spot: " + str("{:,.0f}".format(spotPrice)))
